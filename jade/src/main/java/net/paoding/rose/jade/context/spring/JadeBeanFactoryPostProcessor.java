...
/**
 * {@link JadeBeanFactoryPostProcessor}配置在发布包下的applicationContext-jade.xml中，
 * Spring容器完成其内部的标准初始化工作后将调用本处理器，
 * 识别符合Jade规范的DAO接口并将之配置为Spring容器的Bean定义，加入到Spring容器中。
 * <p>
 * <h1>=开关属性的设置=</h1>
 * <p>
 * 
 *   jade在发布jar包时，将在发布包下的applicationContext-jade.xml文件配置本处理器
 * （也就说本类一定会被Spring容器执行），为使jade能够适应不同的应用环境和业务需要，
 * jade特提供了一些系统属性设定约定，使得您可灵活地控制本处理器的行为，甚至将本处理器视为一个空处理器：
 * <p>
 * 
 ...
 *   并以此类推，直至jade.context.spring.*。这类属性在Jade统称为开关属性。
 * <p>
 * 
 * <strong>jade.context.spring.*</strong><br>
 *   这个属性是所有开关属性的根，即类似jade.context.spring.com和jade.context.spring.cn之类的开关属性，
 * 它的父亲是jade.context.spring.*, 而非jade.context.spring<br>
 *   如果没有设置这个根属性，jade将等价于其被设置为1。
 *   您可以将之设置为0，这样就表示只有那些明确设置了开关属性为1的package或接口的类才由本处理器负责读取并放到Spring容器中。
 * <p>
 * 
 * <h1>=DAO的发现=</h1>
 * <p>
 ...
 * <ul>
 * <li>DAO接口的package必须含有dao目录，如：dao.UserDAO、myapp.dao.UserDAO、myapp.dao.blog.BlogDAO</li>
 ...
 * 接口为DAO方法提供数据源，在本处理器所初始化的spring容器中，数据源的设置有两种方式：
 * 
 * <h2>==定制方式==</h2><br>
 * <ul>
 *   <li>当spring容器配置了一个id/name为"jade.dataSourceFactory"对象，
 *   jade将把这个bean取出来，作为{@link DataSourceFactory}为DAO提供数据源；</li>
 *   <li>当spring容器没有id/name为 "jade.dataSourceFactory"的对象，
 *   但是配置其它名字的{@link DataSourceFactory}，jade将把这个bean 取出来，为DAO提供数据源；</li>
 *   <li>当spring容器没有id/name为"jade.dataSourceFactory"的对象，
 *   但其中存在{@link DataSourceFactory}的个数超过1个，此时系统初始化的时侯不会跑出异常，
 *   但一旦开始进行进行DAO操作时，将抛出IllegalStateException 异常。
 *   （参见 {@link SpringDataSourceFactoryDelegate}）</li>
 * </ul>
 * 
 * <h2>==默认方式==</h2><br>
 *   当spring容器没有配置任何{@link DataSourceFactory}时，jade将启用默认方式为DAO配置数据源，
 * 即使用{@link SpringDataSourceFactory}为DAO提供数据源，从spring容器中寻找对应的数据源。
 *   对于给定的一个DAO接口，如com.mycompany.myapp.dao.UserDAO, 其规则如下：
 * <p>
 * <ul>
 *   <li>如果存在id/name为jade.dataSource.com.mycompany.myapp.dao.UserDAO的数据源，
 *   则使用它作为这个DAO的数据源，否则逐级询问配置，直到顶一级包名：jade.dataSource.com</li>
 *   <li>如果以上仍未能确定UserDAO的数据源，且UserDAO接口上的<code>@DAO</code>的catalog属性非空
 *   （假设其值为myteam.myapp），则视myteam.myapp等同于package名，执行前一个步骤的问询</li>
 *   <ul>
 *     <li>即按此顺序问询Spring容器的配置：jade.dataSource.myteam.myapp.UserDAO，...，jade.dataSource.myteam</li>
 *   </ul>
 *   <li>如果以上仍未能确定UserDAO的数据源，则判断是否存在id/name为jade.dataSource、dataSource的数据源。</li>
 ...
 * 本处理器使用的{@link InterpreterFactory}是{@link SpringInterpreterFactory}。<br>
